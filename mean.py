...
dir_out = './Results Mean/acer_pensylvanicum/'
...
for file_name in my_list:
    ...
    img_bin, mean = threshold_mean(image)
    ...
    a = fig.add_subplot(3,3,2); plt.imshow(img_bin, cmap='gray'); a.set_title('Mean'); a.axis('off')
    a = fig.add_subplot(3,3,3); plt.imshow(img_close, cmap='gray'); a.set_title('Closing'); a.axis('off')
    a = fig.add_subplot(3,3,4); plt.imshow(img_aux, cmap='gray'); a.set_title('Mean - Closing'); a.axis('off')
